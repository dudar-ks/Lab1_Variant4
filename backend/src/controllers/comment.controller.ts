...
import type { CreateCommentRequestDto, UpdateCommentRequestDto } from "../dtos/comments.dto";

export async function getComments(req: Request, res: Response, next: NextFunction) {
  try {
    const postId = typeof req.query.postId === "string" ? Number(req.query.postId) : undefined;
...
export async function getCommentById(
  ...
  try {
    ...
    return res.status(200).json({ item: comment });
  } catch (error) {
    next(error);
  }
}
...
export async function createComment(
  req: Request<Record<string, never>, Record<string, never>, CreateCommentRequestDto>,
  res: Response,
  next: NextFunction
) {
  try {
...
export async function updateComment(
  req: Request<{ id: string }, Record<string, never>, UpdateCommentRequestDto>,
  res: Response,
  next: NextFunction
) {
  try {
