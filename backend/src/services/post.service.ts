...
export async function getPostByIdForUser(id: number, currentUserId: number) {
  const post = await postsRepository.getPostByIdForUser(id, currentUserId);
...
export async function updatePostForUser(
  ...
  try {
    const updatedPost = await postsRepository.updatePostForUser(
      id,
      currentUserId,
      dto.title.trim(),
      dto.category.trim(),
...
export async function deletePostForUser(
  ...
  const deleted = await postsRepository.deletePostForUser(id, currentUserId);
...
export async function getPostWithAuthorForUser(
  id: number,
  currentUserId: number
) {
  const post = await postsRepository.getPostWithAuthorForUser(id, currentUserId);
